...
class Annealing(KerasCallback):
    def __init__(self, weight, start_value=0, final_value=1, epoch_start=0.5):
        """
        ...
        self.epoch_to_start = epoch_start
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        if epoch >= self.epoch_to_start:
            K.set_value(self.weigth, self.start_value + (epoch - self.epoch_to_start) * self.delta)

    def compute_epoch_to_start(self):
        if isinstance(self.epoch_to_start, float):
            self.epoch_to_start = int(self.epoch_to_start * self.epochs)

    def update_with_fit_args(self, **kwargs):
        self.epochs = kwargs['epochs']
        self.compute_epoch_to_start()
        nb_active_epochs = self.epochs - self.epoch_to_start
