...
class BandPlayer(Controller):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        self.arrays.extend([np.zeros((self.step_length, 128)) for _ in range(model.nb_steps)])

        self.model_outputs = [
            np.zeros((self.model.nb_instruments, 1, 1, self.step_length, self.input_size, 1)) for _ in
        ...
        self.current_model_part = np.zeros((self.model.nb_instruments, 1, 1, self.step_length, self.input_size, 1))
        # What will be played by the model on the next step
        self.next_model_part = np.zeros((self.model.nb_instruments, 1, 1, self.step_length, self.input_size, 1))

    @property
    def input_size(self):
        return self.model.notes_range[1] - self.model.notes_range[0]
    ...
    def feed_model(self):
        ...
        played_inst_input = np.concatenate(self.arrays[:-self.model.nb_steps], axis=1)
        # played_inst_input: (nb_steps * step_length, 128)
        played_inst_input = np.reshape(played_inst_input[:, self.model.notes_range[0]: self.model.notes_range[1]],
                                       newshape=(1, self.model.nb_steps, self.step_length, self.input_size, 1))
        # played_inst_input: (batch=1, nb_steps, step_length, input_size, 1
        inputs_models[0] = played_inst_input
        outputs_model = self.model.generate(x=list(inputs_models))
        # output_model: (nb_instruments, batch=1, np_steps=1, step_length, input_size, channels)
        self.next_model_part = outputs_model
